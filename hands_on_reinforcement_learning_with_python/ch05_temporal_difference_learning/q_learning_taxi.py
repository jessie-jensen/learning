...
def update_q_via_q_learning(q, env, state, action, reward, next_state, learning_rate, discount_rate):
    # get next state max q value
    q_max_next_state = max([q[(next_state, a)] for a in range(env.action_space.n)])
    # update current state-action q value:
    # q(s,a) + a(r + g*max[q(s',a')] - q(s,a))
    q[(state, action)] += learning_rate * (reward + (discount_rate * q_max_next_state) - q[(state, action)])
...
def q_learner(env, learning_rate, discount_rate, explore_rate, episodes, episode_render_l=[]):
    q = init_q_table(env)
    ...
    for i in range(episodes+1):
        ...
        while done==False:
            ...
            # update q
            q = update_q_via_q_learning(q, env, state, action, reward, next_state, learning_rate, discount_rate)
            # prep for next state
            state = next_state
            total_reward += reward
            turns += 1
...
if __name__ == "__main__":
    q_learner(env, LEARNING_RATE, DISCOUNT_RATE, EXPLORE_RATE, 10000, [10, 100, 1000, 10000])